  ...
  return (
    <div className="max-w-5xl space-y-5">
      <div className="rounded-2xl overflow-hidden border border-slate-100 bg-white shadow-sm">
        <div className="h-1 bg-gradient-to-r from-rose-400 to-red-300" />
        <div className="flex items-center justify-between gap-4 px-5 py-4">
          <div>
            <h2 className="text-lg font-bold text-slate-900">休暇管理</h2>
            {pendingCount > 0 ? (
              <p className="mt-0.5 text-xs font-medium text-rose-500">未承認の申請が {pendingCount} 件あります</p>
            ) : (
              <p className="mt-0.5 text-sm text-slate-500">休暇申請の承認・管理を行います。</p>
            )}
          </div>
      ...
      <div className="space-y-3 sm:hidden">
        {filtered.length === 0 && !loading ? (
          <div className="rounded-2xl border border-slate-100 bg-white px-5 py-10 text-center text-sm text-slate-400 shadow-sm">
            該当する申請がありません。
          </div>
        ) : null}
        ...
        {filtered.map((req) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-slate-900">{req.employeeName}</p>
                <p className="mt-1 text-xs text-slate-400">{req.department}</p>
              </div>
              <span className={`shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${STATUS_COLOR[req.status] ?? ""}`}>
                {req.status}
              </span>
            </div>
            ...
            <div className="mt-4 space-y-2 text-xs text-slate-600">
              <div className="flex items-center justify-between gap-3">
                <span className="text-slate-400">休暇種類</span>
                <span className="truncate text-right font-medium text-slate-700">{req.leaveType}</span>
              </div>
              <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center justify-between gap-3">
                <span className="text-slate-400">日数</span>
                <span className="font-medium text-slate-700">{req.days}日</span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-slate-400">申請日</span>
                <span className="font-mono text-slate-500">{req.appliedAt}</span>
              </div>
            </div>
                ...
                req.status === "待機中" ? (
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 rounded-lg border border-emerald-200 px-3 py-2 text-xs font-semibold text-emerald-600 transition-colors hover:bg-emerald-50"
                    >
                      承認
                    </button>
                    <button
      ...
      <div className="hidden overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm sm:block">
        <div className="border-b border-rose-100 bg-gradient-to-r from-rose-50/60 to-white px-5 py-3">
          <p className="text-xs font-semibold text-rose-600">{filtered.length} 件</p>
        </div>
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-rose-100 bg-gradient-to-r from-rose-50/60 to-white">
                  <th className="px-5 py-3 text-left text-xs font-semibold text-rose-600">氏名</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-rose-600 hidden sm:table-cell">部署</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-rose-600">休暇種類</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-rose-600 hidden md:table-cell">期間</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-rose-600 hidden lg:table-cell">日数</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold text-rose-600">状態</th>
                  <th className="hidden px-5 py-3 text-xs font-semibold text-rose-600 text-right sm:table-cell">操作</th>
                </tr>
              </thead>
